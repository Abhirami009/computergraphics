...
{
  std::string vertex_shader_file("vs_blend.glsl");
  std::string fragment_shader_file("fs_blend.glsl");
...
void processInput(GLFWwindow* window) {
    if (glfwGetKey(window, GLFW_KEY_RIGHT) == GLFW_PRESS) rot_angle_x -= 1.0f;
    if (glfwGetKey(window, GLFW_KEY_LEFT) == GLFW_PRESS) rot_angle_x += 1.0f;
    if (glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS) rot_angle_y += 1.0f;
    if (glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS) rot_angle_y -= 1.0f;
    if (glfwGetKey(window, GLFW_KEY_U) == GLFW_PRESS) rot_angle_z += 1.0f;
    if (glfwGetKey(window, GLFW_KEY_B) == GLFW_PRESS) rot_angle_z -= 1.0f;  // Corrected from GLFW_KEY_D
}
...
void initTextures() {
    texture1 = loadBMP("texture.bmp");  // Load first texture
    texture2 = loadBMP("texture2.bmp");  // Load second texture
}
