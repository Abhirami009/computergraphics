...
void generateSphere(float radius, int sectorCount, int stackCount) {
    ...
    for (int i = 0; i <= stackCount; ++i) {
        ...
        for (int j = 0; j <= sectorCount; ++j) {
            ...
            // Add vertex color (for base color reference)
...
void initShadersGL(void) {
    std::string vertex_shader_file("gourad_vs.glsl");
    std::string fragment_shader_file("gourad_fs.glsl");
...
int main(int argc, char** argv)
{
    ...
    window = glfwCreateWindow(640, 480, "Rotating Sphere -- gourad shading", NULL, NULL);
    if (!window) {
        glfwTerminate();
        return -1;
    }
