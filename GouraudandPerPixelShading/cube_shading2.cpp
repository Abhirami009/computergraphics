...
void generateSphere(float radius, int sectorCount, int stackCount) {
    ...
    for (int i = 0; i <= stackCount; ++i) {
        ...
        for (int j = 0; j <= sectorCount; ++j) {
            ...
            vertices_global.push_back(normal.x);
            vertices_global.push_back(normal.y);
            vertices_global.push_back(normal.z);

            // Assign color based on position
            vertices_global.push_back((x + radius) / (2 * radius)); // R
            vertices_global.push_back((y + radius) / (2 * radius)); // G
            vertices_global.push_back((z + radius) / (2 * radius)); // B
        }
    }
...
void renderGL(GLFWwindow* window) {
    ...
    glUniform3f(viewPosLoc, 15.0f, 15.0f, 15.0f);
...
int main(int argc, char** argv)
{
    ...
    window = glfwCreateWindow(640, 480, "Rotating Sphere -- per-pixel shading", NULL, NULL);
    if (!window) {
        glfwTerminate();
        return -1;
    }
